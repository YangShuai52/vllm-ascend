...
class PrecopyMambaAlignV310 : public OpDef {
public:
    explicit PrecopyMambaAlignV310(const char* name) : OpDef(name)
    {
        this->Input("stateIdx").ParamType(REQUIRED).DataType({ge::DT_INT32}).Format({ge::FORMAT_ND});
        this->Input("srcCol").ParamType(REQUIRED).DataType({ge::DT_INT32}).Format({ge::FORMAT_ND});
        this->Input("tokenBias").ParamType(REQUIRED).DataType({ge::DT_INT32}).Format({ge::FORMAT_ND});
        ...
        this->Output("stateIdxOut").ParamType(REQUIRED).DataType({ge::DT_INT32}).Format({ge::FORMAT_ND});
